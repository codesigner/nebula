...
uint32_t Cpp2Ops<nebula::Edge>::write(Protocol* proto, nebula::Edge const* obj) {
  ...
  // NOTICE: The original id will be transformed to +1/-1, to indicate the edge
  // direction.
  auto type = obj->type;
  if (type != 0) {
    type > 0 ? type = 1 : type = -1;
  }
  xfer += detail::pm::protocol_methods<type_class::integral, nebula::EdgeType>::write(*proto, type);
  xfer += proto->writeFieldEnd();
...
_readField_src : { ::apache::thrift::Cpp2Ops<nebula::Value>::read(proto, &obj->src); }

  if (UNLIKELY(!readState.advanceToNextField(proto, 1, 2, protocol::T_STRUCT))) {
    goto _loop;
  }

_readField_dst : { ::apache::thrift::Cpp2Ops<nebula::Value>::read(proto, &obj->dst); }
